...
async def send_control_command(
    ...
    """
    Send a control command to a device via its PC Client
    
    The command will be routed to the PC Client that manages this device.
    If the client is offline, the command will be queued for later delivery.
    """
    ...
    # Check if device has a client assigned
    if not device.client_id:
        # Device not assigned to any client yet - queue for later
        logger.warning(f"Device {device_id} has no client assigned")
        return command
    
    # Try to send to PC client if it's online
    from app.services.client_connection_manager import client_manager
    
    if client_manager.is_online(device.client_id):
        # Client is online - send command immediately
        command_dict = {
            "type": "device_command",
        ...
        success = await client_manager.send_command(device.client_id, command_dict)
        if success:
            await CommandQueue.mark_command_sent(db, command.id)
            await db.refresh(command)
            logger.info(f"Command {command.id} sent to online client {device.client_id}")
    else:
        # Client is offline - command will be queued automatically
        logger.info(f"Command {command.id} queued for offline client {device.client_id}")
